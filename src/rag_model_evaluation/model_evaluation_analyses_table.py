...
for model in config['Models']['list']:
    review_list = []
    for i in range(0,3):
        cur.execute(f"SELECT COUNT(*) FROM model_evaluation_rag WHERE review={i} AND model='{model}';")
...
code = df.to_latex(index=True, escape=True, column_format="|l|c|c|c|", caption=r"Model evaluation by model with RAG")
...
for j in range(0, 3):
        ...
        for model in config['Models']['list']:
                tmp_list = []
                for i in range(0,3):
                    cur.execute(f"SELECT COUNT(*) FROM model_evaluation_rag WHERE model='{model}' AND review={i} AND class='{classes[j]}';")
                    eval = cur.fetchone()[0]
...
code = df_end.to_latex(index=True, escape=True, column_format="|c|c|c|c|", caption=r"Model evaluation by model with RAG")
